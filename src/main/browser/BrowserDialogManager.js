...
exports.BrowserDialogManager = void 0;
...
`;
class BrowserDialogManager {
    pendingDialogs = new Map();
    promptDialogResolutions = new Map();
    dialogDebuggerTabs = new Set();
    deps;
    constructor(deps) {
        this.deps = deps;
    }
    async installPromptShimInPage(entry) {
    ...
    ensureDebugger(entry) {
        if (this.dialogDebuggerTabs.has(entry.id))
            return;
        const wc = entry.view.webContents;
    ...
    clearPendingDialogsForTab(tabId) {
        let changed = false;
        for (const [id, dialog] of this.pendingDialogs.entries()) {
            if (dialog.tabId !== tabId)
                continue;
    ...
    async acceptDialog(input = {}, activeTabId) {
        return this.resolveJavaScriptDialog({ ...input, accept: true }, activeTabId);
    }
    async dismissDialog(input = {}, activeTabId) {
    ...
    normalizeJavaScriptDialogType(value) {
        switch (value) {
            case 'alert':
            case 'confirm':
            case 'prompt':
    ...
    async resolveJavaScriptDialog(input, activeTabId) {
...
exports.BrowserDialogManager = BrowserDialogManager;
//# sourceMappingURL=BrowserDialogManager.js.map