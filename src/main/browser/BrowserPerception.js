...
const surfaceResolver_1 = require("../../shared/browser/surfaceResolver");
const ids_1 = require("../../shared/utils/ids");
function normalizeActionable(raw, tabId) {
    return {
        id: raw.id || (0, ids_1.generateId)('act'),
        ref: {
...
function normalizeField(raw, tabId) {
    return {
        id: raw.id || (0, ids_1.generateId)('field'),
        ref: {
            tabId: typeof raw.tabId === 'string' && raw.tabId ? raw.tabId : tabId,
            frameId: typeof raw.frameId === 'string' ? raw.frameId : null,
            selector: raw.selector || '',
...
function normalizeForm(raw, tabId) {
    return {
        id: raw.id || (0, ids_1.generateId)('form'),
        ...
        action: raw.action || '',
        fields: Array.isArray(raw.fields) ? raw.fields.map((f) => normalizeField(f, tabId)) : [],
        submitLabels: Array.isArray(raw.submitLabels) ? raw.submitLabels : [],
    };
}
class BrowserPerception {
    executeInPage;
    constructor(executeInPage) {
        this.executeInPage = executeInPage;
    }
    async captureTabSnapshot(tabId, strategy) {
    ...
    async getActionableElements(tabId, strategy) {
        const { payload } = await this.capturePerception(tabId, strategy || null);
        return payload.actionableElements;
    }
    async getFormModel(tabId, strategy) {
        const { payload } = await this.capturePerception(tabId, strategy || null);
        return payload.forms;
    }
    async exportSurfaceEvalFixture(tabId, name, strategy) {
    ...
    async capturePerception(tabId, strategy) {
    ...
    `, tabId);
        ...
        const evidence = {
            ...
            panelCandidates: Array.isArray(raw.panelCandidates) ? raw.panelCandidates.map((panel) => ({
                selector: panel.selector || '',
                label: panel.label || '',
        ...
        const resolved = (0, surfaceResolver_1.resolveBrowserSurface)(evidence);
        const payload = {
            url: raw.url || '',
            title: raw.title || '',
            mainHeading: raw.mainHeading || '',
            ...
            actionableElements: Array.isArray(raw.actionableElements)
                ? raw.actionableElements.map((item) => normalizeActionable(item, tabId))
                : [],
            forms: Array.isArray(raw.forms)
                ? raw.forms.map((item) => normalizeForm(item, tabId))
                : [],
        };
...
//# sourceMappingURL=BrowserPerception.js.map