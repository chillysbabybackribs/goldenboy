...
exports.BrowserPageInteraction = void 0;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const electron_1 = require("electron");
class BrowserPageInteraction {
    resolveEntry;
    constructor(resolveEntry) {
    ...
    async executeInPage(expression, tabId) {
        ...
        if (!entry)
            return { result: null, error: 'No active tab' };
        try {
    ...
    // ─── Upload File ────────────────────────────────────────────────────────
    async uploadFile(selector, filePath, tabId) {
        const entry = tabId ? this.resolveEntry(tabId) : this.resolveEntry();
        if (!entry)
            return { uploaded: false, error: 'No active tab' };
    ...
    `);
        ...
        try {
            ...
            const documentRoot = await dbg.sendCommand('DOM.getDocument', { depth: -1, pierce: true });
    ...
    `, tabId);
        if (post.error)
            return { uploaded: false, error: post.error };
    ...
    async clickElement(selector, tabId) {
        const entry = tabId ? this.resolveEntry(tabId) : this.resolveEntry();
        if (!entry)
            return { clicked: false, error: 'No active tab' };
        const safeSelector = JSON.stringify(selector);
    ...
    `, tabId);
        if (error)
            return { ok: false, error, selector };
        const raw = (result || {});
        if (raw.ok !== true) {
            return {
                ok: false,
    ...
    async hoverElement(selector, tabId) {
        const entry = tabId ? this.resolveEntry(tabId) : this.resolveEntry();
        if (!entry)
            return { hovered: false, error: 'No active tab' };
        const hitTest = await this.hitTestElement(selector, tabId);
    ...
    `, tabId);
        if (error)
            return { hovered: false, error, hitTest };
        const r = result;
        return {
    ...
    async dragElement(sourceSelector, targetSelector, tabId) {
        const entry = tabId ? this.resolveEntry(tabId) : this.resolveEntry();
        if (!entry)
            return { dragged: false, error: 'No active tab' };
    ...
    toGlobalPoint(entry, x, y) {
        const viewBounds = entry.view.getBounds();
        const windowBounds = electron_1.BrowserWindow.fromWebContents(entry.view.webContents)?.getBounds();
        return {
...
exports.BrowserPageInteraction = BrowserPageInteraction;
//# sourceMappingURL=BrowserPageInteraction.js.map