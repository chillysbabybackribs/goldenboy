...
exports.decideBrowserExecution = decideBrowserExecution;
exports.finalizeBrowserExecutionDecision = finalizeBrowserExecutionDecision;
function networkAssessmentForOperation(kind) {
    switch (kind) {
...
function confidenceForMode(mode, validation) {
    if (mode === 'abort')
        return 'low';
...
function decideBrowserExecution(input) {
    ...
    if (input.supportsDeterministicExecution)
        evidence.push('deterministic-supported');
    else
        evidence.push('deterministic-unsupported');
    if (input.replayOfOperationId)
        evidence.push('replay-requested');
    if (validation)
        evidence.push(`preflight:${validation.status}`);
...
function finalizeBrowserExecutionDecision(decision, input) {
...
//# sourceMappingURL=browserExecutionPolicy.js.map