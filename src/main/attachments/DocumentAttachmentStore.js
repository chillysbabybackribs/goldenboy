...
exports.documentAttachmentStore = exports.DocumentAttachmentStore = void 0;
const crypto = __importStar(require("crypto"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const electron_1 = require("electron");
const FileChunker_1 = require("../fileKnowledge/FileChunker");
...
function storeRoot() {
    return path.join(electron_1.app.getPath('userData'), STORE_DIR);
}
function indexPath() {
...
function normalizeMediaType(filePath, provided) {
    if (provided && provided.trim())
        return provided.trim();
    return MEDIA_TYPES_BY_EXTENSION[path.extname(filePath).toLowerCase()] || 'application/octet-stream';
}
function isTextExtractable(filePath, mediaType) {
    const ext = path.extname(filePath).toLowerCase();
    if (TEXT_EXTRACTABLE_EXTENSIONS.has(ext))
        return true;
...
function buildChunks(attachmentId, taskId, text) {
    ...
    while (startLine <= lines.length) {
        ...
        chunks.push({
            ...
            tokenEstimate: (0, FileChunker_1.estimateTokens)(chunkText),
            text: chunkText,
        });
        startLine = endLine + 1;
        ordinal++;
...
class DocumentAttachmentStore {
    ...
    async importDocuments(taskId, documents) {
        if (!documents.length)
            return [];
        ensureStoreDirs();
        const imported = [];
        for (const input of documents) {
            ...
            let charCount = 0;
            let lineCount = 0;
            const language = (0, FileChunker_1.languageForPath)(name);
            let builtChunks = [];
            if (isTextExtractable(sourcePath, mediaType)) {
                if (stat.size > MAX_TEXT_EXTRACT_BYTES) {
                    ...
                    try {
                        ...
                        tokenEstimate = (0, FileChunker_1.estimateTokens)(text);
    ...
    search(taskId, query, input) {
        const terms = normalizeQuery(query);
        if (terms.length === 0)
            return [];
        const results = [];
        for (const document of this.documents.values()) {
            if (document.taskId !== taskId || document.status !== 'indexed')
                continue;
            for (const chunkId of document.chunkIds) {
                ...
                results.push({
                    ...
                    score,
                    tokenEstimate: Math.min(chunk.tokenEstimate, (0, FileChunker_1.estimateTokens)(chunk.text)),
                });
            }
        }
        return results
    ...
    readChunk(taskId, chunkId, maxChars = 3000) {
        const chunk = this.chunks.get(chunkId);
        if (!chunk || chunk.taskId !== taskId)
            return null;
    ...
    readDocument(taskId, documentId, maxChars = 4000) {
    ...
    findByTaskAndHash(taskId, contentHash) {
        for (const document of this.documents.values()) {
    ...
    toInvocationAttachment(document) {
        return {
            type: 'document',
            id: document.id,
            name: document.name,
...
exports.DocumentAttachmentStore = DocumentAttachmentStore;
exports.documentAttachmentStore = new DocumentAttachmentStore();
//# sourceMappingURL=DocumentAttachmentStore.js.map