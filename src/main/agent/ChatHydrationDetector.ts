...
export class ChatHydrationDetector {
  ...
  extractContextKeywords(userMessage: string): string[] {
    ...
    if (theMatches) {
      theMatches.forEach((m) => {
        ...
        if (term.length >= 3 && !this.isStopWord(term)) terms.add(term);
      });
    }
   ...
   */
  private looksLikeFollowUp(lower: string): boolean {
   ...
   */
  private hasFollowUpSignal(lower: string): boolean {
   ...
   */
  private hasRelativeContext(lower: string): boolean {
   ...
   */
  private hasTopicReferences(lower: string): boolean {
   ...
   */
  private matchesSet(lower: string, set: Set<string>): boolean {
...
export const chatHydrationDetector = new ChatHydrationDetector();
