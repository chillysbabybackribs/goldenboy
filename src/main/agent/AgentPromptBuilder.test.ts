...
describe('buildResponseStyleAddendum', () => {
  it('does not apply browser-task response rules to browser-routing discussion', () => {
    const addendum = buildResponseStyleAddendum(
      'Change the prompt routing so the word browser does not auto-trigger a browser task',
    );
  ...
  it('keeps browser-task response rules for explicit browser actions', () => {
    const addendum = buildResponseStyleAddendum(
      'Please open a new browser tab and go to https://example.com',
    );
