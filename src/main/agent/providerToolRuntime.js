...
exports.MAX_PROVIDER_TOOL_TURNS = exports.DEFAULT_PROVIDER_MAX_TOOL_TURNS = void 0;
exports.normalizeProviderMaxToolTurns = normalizeProviderMaxToolTurns;
exports.describeProviderToolCall = describeProviderToolCall;
exports.resolveRuntimeToolExpansion = resolveRuntimeToolExpansion;
exports.applyRuntimeToolExpansion = applyRuntimeToolExpansion;
exports.applyAutoExpandedToolPack = applyAutoExpandedToolPack;
exports.formatQueuedExpansionLines = formatQueuedExpansionLines;
exports.formatAutoExpandedToolPackLines = formatAutoExpandedToolPackLines;
exports.encodeToolInput = encodeToolInput;
exports.normalizeProviderFinalOutput = normalizeProviderFinalOutput;
exports.publishProviderFinalOutput = publishProviderFinalOutput;
exports.executeProviderToolCall = executeProviderToolCall;
exports.executeProviderToolCallWithEvents = executeProviderToolCallWithEvents;
...
exports.DEFAULT_PROVIDER_MAX_TOOL_TURNS = 20;
exports.MAX_PROVIDER_TOOL_TURNS = 40;
...
function normalizeProviderMaxToolTurns(requestedTurns) {
    return Math.min(Math.max(Math.floor(requestedTurns ?? exports.DEFAULT_PROVIDER_MAX_TOOL_TURNS), 1), exports.MAX_PROVIDER_TOOL_TURNS);
}
function describeProviderToolCall(toolName, input) {
...
function resolveRuntimeToolExpansion(request, currentTools, toolName, result) {
    ...
    if (toolName === 'runtime.request_tool_pack') {
        ...
        if (!pack)
            return null;
        const expansion = (0, toolPacks_1.resolveRequestedToolPack)(pack, request.toolCatalog);
        if (!expansion)
            return null;
    ...
    if (toolName !== 'runtime.search_tools')
        return null;
    ...
    const matched = resultTools
        .filter((name) => typeof name === 'string' && catalogNames.has(name));
...
function applyRuntimeToolExpansion(input) {
    const expansion = resolveRuntimeToolExpansion(input.request, input.toolBindingStore.getCallableTools(), input.toolName, input.result);
    if (!expansion)
        return null;
    input.toolBindingStore.queueTools(expansion.tools);
    return expansion;
}
function applyAutoExpandedToolPack(input) {
    const expansion = (0, toolPacks_1.resolveAutoExpandedToolPack)(input.message, input.toolBindingStore.getCallableTools(), input.toolCatalog);
    if (!expansion)
        return null;
    input.toolBindingStore.queueTools(expansion.tools);
    return expansion;
}
function expansionToolNames(expansion) {
    return expansion.scope === 'all'
        ? ['all eligible tools']
        : expansion.tools;
}
function formatQueuedExpansionLines(expansion, options) {
...
function formatAutoExpandedToolPackLines(expansion, options) {
    const expandedToolNames = expansionToolNames(expansion);
    const lines = [
    ...
    if (options?.includeCallableStatus) {
        lines.push('Callable now: none newly added in this turn', `Callable next turn: ${expandedToolNames.join(', ')}`);
    }
...
function encodeToolInput(value) {
    return JSON.stringify(value, null, 2);
}
function normalizeProviderFinalOutput(text) {
    return text.trim()
        ? text
        : DEFAULT_PROVIDER_EMPTY_RESPONSE;
}
function publishProviderFinalOutput(input) {
    const item = {
        id: input.itemId,
        type: 'agent_message',
...
async function executeProviderToolCall(input) {
    try {
        const currentTools = input.currentTools ?? (0, toolBindingScope_1.listCallableRequestTools)(input.request);
        const toolCatalog = input.request.toolCatalog;
        const result = await AgentToolExecutor_1.agentToolExecutor.execute(input.toolName, input.toolInput, {
...
async function executeProviderToolCallWithEvents(input) {
...
function recordToolMemory(input, outcome) {
    if (!input.request.taskId || input.toolName.startsWith('chat.'))
        return;
    ChatKnowledgeStore_1.chatKnowledgeStore.recordToolMessage(input.request.taskId, serializeToolMemory({
        toolName: input.toolName,
        toolInput: input.toolInput,
        result: outcome.result,
        error: outcome.error,
    }), input.providerId, input.request.runId);
}
function createProviderToolCallItem(itemId, toolName, toolInput) {
    return {
        id: itemId,
        type: 'mcp_tool_call',
        server: 'v2',
        tool: toolName,
        arguments: (toolInput && typeof toolInput === 'object') ? toolInput : {},
        result: null,
        error: null,
        status: 'in_progress',
    };
...
function formatToolResultForModel(result) {
    ...
    if (result.validation) {
        toolContent += (0, ConstraintValidator_1.formatValidationForModel)(result.validation);
    }
    return toolContent;
}
...
function describeProviderToolResult(result, isError) {
    if (isError) {
...
function enrichToolSummary(summary, data) {
    ...
    if (Array.isArray(data.matches)) {
        const preview = data.matches
            ...
            if (match && typeof match === 'object' && typeof match.path === 'string') {
                return path_1.default.basename(match.path);
            }
            return null;
        })
            .filter((value) => Boolean(value));
...
function terminalOutputSnippet(output) {
...
function previewTitleOrUrl(value) {
...
//# sourceMappingURL=providerToolRuntime.js.map