...
exports.formatValidationForModel = formatValidationForModel;
// --- Deterministic checks ---------------------------------------------------
function checkExitCode(result) {
...
function checkOutputContainsError(result) {
...
function checkCreationVerb(result, input) {
...
function checkOwnershipFromGhOutput(result, input) {
    const command = extractCommand(input);
    if (!command)
        return null;
    // Only applies to gh repo commands
    if (!/\bgh\s+repo\b/i.test(command))
        return null;
    const output = typeof result.data.output === 'string' ? result.data.output : '';
...
function checkBrowserNavigationTarget(result, input) {
...
function checkResearchEvidenceSufficiency(result) {
    const openedPages = result.data.openedPages;
    if (!Array.isArray(openedPages))
        return null;
    const anyLikely = openedPages.some((p) => p.answerLikely === true);
    const allScores = openedPages
        .map((p) => typeof p.evidenceScore === 'number' ? p.evidenceScore : 0);
...
function checkBrowserCreateTab(result) {
    ...
    const tabId = typeof tab.id === 'string'
        ? tab.id
        : '';
    const present = !!tabId && tabs.some((entry) => entry && typeof entry === 'object' && entry.id === tabId);
    return {
        name: 'tab_created',
...
function isRetainedHomepageTab(entry, homepage) {
    if (!entry || typeof entry !== 'object')
        return false;
    const navigation = entry.navigation;
    if (!navigation || typeof navigation !== 'object')
        return false;
    const navigationRecord = navigation;
...
function checkBrowserClosedTabs(result, input) {
    const obj = typeof input === 'object' && input !== null ? input : {};
    const requested = [
        ...(typeof obj.tabId === 'string' ? [obj.tabId] : []),
        ...(Array.isArray(obj.tabIds) ? obj.tabIds.filter((value) => typeof value === 'string') : []),
    ];
    ...
    const survivors = requested.filter((tabId) => tabs.some((entry) => entry && typeof entry === 'object' && entry.id === tabId));
...
function checkBrowserActivatedTab(result, input) {
    const obj = typeof input === 'object' && input !== null ? input : {};
...
// --- Classification ---------------------------------------------------------
function classifyResult(constraints) {
    if (constraints.length === 0)
        return 'VALID';
    const hasFail = constraints.some(c => c.status === 'FAIL');
    if (hasFail)
        return 'INVALID';
    const hasUncertain = constraints.some(c => c.status === 'UNKNOWN' || c.status === 'ESTIMATED' || c.status === 'CONDITIONAL');
    if (hasUncertain)
        return 'INCOMPLETE';
    return 'VALID';
}
function buildValidationSummary(status, constraints) {
    if (status === 'VALID')
...
const TERMINAL_EXEC_CONSTRAINTS = (result, input) => {
    const verdicts = [];
    verdicts.push(checkExitCode(result));
    const errorCheck = checkOutputContainsError(result);
    if (errorCheck)
        verdicts.push(errorCheck);
    const createCheck = checkCreationVerb(result, input);
    if (createCheck)
        verdicts.push(createCheck);
    const ownerCheck = checkOwnershipFromGhOutput(result, input);
    if (ownerCheck)
        verdicts.push(ownerCheck);
    return verdicts;
};
const BROWSER_NAVIGATE_CONSTRAINTS = (result, input) => {
    const verdicts = [];
    const navCheck = checkBrowserNavigationTarget(result, input);
    if (navCheck)
        verdicts.push(navCheck);
    return verdicts;
};
const RESEARCH_SEARCH_CONSTRAINTS = (result, _input) => {
    const verdicts = [];
    const evidenceCheck = checkResearchEvidenceSufficiency(result);
    if (evidenceCheck)
        verdicts.push(evidenceCheck);
    return verdicts;
};
const BROWSER_CREATE_TAB_CONSTRAINTS = (result, _input) => {
    const verdicts = [];
...
const BROWSER_CLOSE_TAB_CONSTRAINTS = (result, input) => {
    const verdicts = [];
    const closeCheck = checkBrowserClosedTabs(result, input);
    if (closeCheck)
        verdicts.push(closeCheck);
    return verdicts;
};
const BROWSER_ACTIVATE_TAB_CONSTRAINTS = (result, input) => {
    const verdicts = [];
    const activateCheck = checkBrowserActivatedTab(result, input);
    if (activateCheck)
        verdicts.push(activateCheck);
    return verdicts;
};
// Map tool names to their constraint extractors
const TOOL_CONSTRAINTS = new Map([
...
// --- Public API -------------------------------------------------------------
function validateToolResult(toolName, result, input) {
...
function formatValidationForModel(validation) {
    const lines = [
        '',
