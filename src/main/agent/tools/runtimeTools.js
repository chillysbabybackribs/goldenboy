...
exports.createRuntimeToolDefinitions = createRuntimeToolDefinitions;
...
function validateSchemaValue(value, schema, path) {
    ...
    if (oneOf) {
        const matches = oneOf.some((candidate) => (typeof candidate === 'object'
            && candidate !== null
            && validateSchemaValue(value, candidate, path).length === 0));
    ...
    switch (schemaType) {
        case 'object': {
            ...
            const required = Array.isArray(schema.required)
                ? schema.required.filter((entry) => typeof entry === 'string' && entry.trim().length > 0)
                : [];
    ...
    return [
        {
            ...
            async execute(input, context) {
                ...
                const matches = (0, toolPacks_1.searchToolCatalog)(query, context.toolCatalog, {
        ...
        },
        {
            ...
            async execute(input, context) {
                const obj = objectInput(input);
                const toolName = requireString(obj, 'tool');
                ...
                const result = await AgentToolExecutor_1.agentToolExecutor.execute(toolName, toolInput, context);
                return {
        ...
        },
        {
            ...
            async execute() {
                const packs = (0, toolPacks_1.listToolPacks)().map((pack) => ({
                    id: pack.id,
        ...
        },
        {
            ...
            description: (0, toolPacks_1.buildRuntimeRequestToolDescription)(),
            inputSchema: {
                type: 'object',
                additionalProperties: false,
                required: ['pack'],
                properties: {
                    pack: {
                        type: 'string',
                        enum: (0, toolPacks_1.listToolPacks)().map((pack) => pack.id),
                    },
                    reason: { type: 'string' },
                },
            },
            async execute(input) {
                ...
                const manifest = (0, toolPacks_1.listToolPacks)().find((entry) => entry.id === pack);
                if (!manifest) {
