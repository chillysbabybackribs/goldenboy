...
const SubAgentRuntime_1 = require("./SubAgentRuntime");
...
function toRelativeWorkspacePath(rawPath) {
    const absolute = path.isAbsolute(rawPath) ? rawPath : path.resolve(workspaceRoot_1.APP_WORKSPACE_ROOT, rawPath);
    const relative = path.relative(workspaceRoot_1.APP_WORKSPACE_ROOT, absolute);
    if (!relative || relative.startsWith('..'))
        return rawPath;
    return relative;
}
function toolResultShape(value) {
    if (!value || typeof value !== 'object')
        return null;
    const candidate = value;
    return typeof candidate.summary === 'string' ? candidate : null;
}
function extractFindings(output, toolCalls) {
    ...
    if (bulletsOnly.length > 0)
        return limitList(bulletsOnly, 5);
    const paragraphs = output
        .split(/\n{2,}/)
        .map(part => part.trim().replace(/\s+/g, ' '))
        .filter(Boolean);
    if (paragraphs.length > 0)
        return limitList(paragraphs, 3);
    return limitList(toolCalls
        .map(call => toolResultShape(call.output)?.summary || call.error || '')
        .filter(Boolean), 3);
}
function extractChangedFiles(toolCalls, codexItems) {
    const files = [];
    for (const item of codexItems || []) {
        if (item.type !== 'file_change')
            continue;
        for (const change of item.changes)
            files.push(toRelativeWorkspacePath(change.path));
    }
        ...
        switch (call.toolName) {
            ...
            case 'filesystem.mkdir':
                if (typeof data.path === 'string')
                    files.push(toRelativeWorkspacePath(data.path));
                break;
            case 'filesystem.move':
                if (typeof data.from === 'string')
                    files.push(toRelativeWorkspacePath(data.from));
                if (typeof data.to === 'string')
                    files.push(toRelativeWorkspacePath(data.to));
                break;
            default:
                break;
        }
...
function extractCommands(toolCalls, codexItems) {
    const commands = [];
    for (const item of codexItems || []) {
    ...
    for (const call of toolCalls) {
        if (call.toolName !== 'terminal.exec' && call.toolName !== 'terminal.spawn')
            continue;
        ...
        const exitCode = output?.data && typeof output.data.exitCode === 'number'
            ...
            : '';
        if (base)
            commands.push(`${base}${exitCode}`);
    }
    return limitList(commands, 20);
}
function summarizeValidation(toolCalls) {
...
function summarizeToolCalls(toolCalls) {
    return toolCalls.map((call) => {
        const output = toolResultShape(call.output);
        const validationStatus = output?.validation?.status;
        return {
...
function extractBlockers(status, summary, toolCalls) {
    const blockers = [];
...
class SubAgentManager {
    ...
    run(parentRunId, input) {
        const record = this.spawn(parentRunId, input);
        const runtime = new SubAgentRuntime_1.SubAgentRuntime(this.providerFactory(input));
        const promise = (async () => {
            ...
            const toolCalls = result.runId ? AgentRunStore_1.agentRunStore.listToolCalls(result.runId) : [];
            const subResult = {
                id: record.id,
                status: 'completed',
                summary,
            ...
            const toolCalls = record.runId ? AgentRunStore_1.agentRunStore.listToolCalls(record.runId) : [];
            const subResult = {
                id: record.id,
                status,
                summary: message,
    ...
    spawnBackground(parentRunId, input) {
    ...
    skillNamesForRole(role, canSpawnSubagents) {
        const normalized = role.toLowerCase();
        const skills = canSpawnSubagents ? ['subagent-coordination'] : [];
        if (normalized.includes('browser') || normalized.includes('research'))
            skills.push('browser-operation');
        if (normalized.includes('file') || normalized.includes('code'))
            skills.push('filesystem-operation');
        if (normalized.includes('debug') || normalized.includes('terminal'))
            skills.push('local-debug');
        if (skills.length === 0 || (canSpawnSubagents && skills.length === 1)) {
            skills.push('browser-operation', 'filesystem-operation', 'local-debug');
        }
        return skills;
    }
    contextForSpawn(input) {
        if (!input.taskId || input.inheritedContext === 'none')
            return null;
        const parts = [];
        if (input.inheritedContext === 'full') {
            const chatContext = ChatKnowledgeStore_1.chatKnowledgeStore.buildInvocationContext(input.taskId);
            const taskMemory = taskMemoryStore_1.taskMemoryStore.buildContext(input.taskId);
            if (chatContext)
                parts.push(chatContext);
            if (taskMemory)
                parts.push(taskMemory);
        }
        else {
            const summary = ChatKnowledgeStore_1.chatKnowledgeStore.threadSummary(input.taskId);
            if (summary)
                parts.push(['## Parent Conversation Summary', summary].join('\n'));
        }
    ...
    prune(now = Date.now()) {
        ...
        if (this.records.size <= MAX_SUB_AGENT_RECORDS)
            return;
