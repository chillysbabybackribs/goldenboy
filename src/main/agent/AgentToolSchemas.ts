...
export const UNRESTRICTED_DEV_TOOL_SCHEMAS: AgentToolSchemaSummary[] = [
  ...
  schema('browser.upload_file', 'Attach a local file to an input[type="file"] element using the browser runtime.', { type: 'object', required: ['selector', 'filePath'], properties: { selector: { type: 'string' }, filePath: { type: 'string' }, tabId: { type: 'string' } } }),
  schema('browser.download_link', 'Start a tracked browser download from a page link selector without relying on a normal click/navigation path.', { type: 'object', required: ['selector'], properties: { selector: { type: 'string' }, tabId: { type: 'string' } } }),
  schema('browser.download_url', 'Start a tracked browser download from an explicit URL.', { type: 'object', required: ['url'], properties: { url: { type: 'string' }, tabId: { type: 'string' } } }),
  schema('browser.get_downloads', 'Return current and completed browser downloads.', { type: 'object', properties: { state: { type: 'string' }, filename: { type: 'string' }, tabId: { type: 'string' } } }),
  schema('browser.wait_for_download', 'Wait for a browser download to complete or settle.', { type: 'object', properties: { downloadId: { type: 'string' }, filename: { type: 'string' }, tabId: { type: 'string' }, timeoutMs: { type: 'number' } } }),
  ...
  schema('browser.run_intent_program', 'Execute semantic Web Intent VM bytecode for login/dialog/hover/drag-drop/cart/checkout/upload/extract/assert flows.', {
    type: 'object',
