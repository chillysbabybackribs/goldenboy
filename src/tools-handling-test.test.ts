...
describe('tools handling diagnostics', () => {
  it('keeps a stable mock tool inventory for local audits', () => {
    expect(ALL_MOCK_TOOLS).toHaveLength(20);
    expect(ALL_MOCK_TOOLS[0].name).toBe('browser.get_state');
    expect(ALL_MOCK_TOOLS.at(-1)?.name).toBe('chat.search');
  });

  it('shows the relative size of the codex-style tool planning surface', () => {
