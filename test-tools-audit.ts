...
import { scopeForPrompt } from './src/main/agent/runtimeScope';
...
for (const task of tasks) {
  const scope = scopeForPrompt(task.prompt);
  ...
  console.log(`  Scope result: allowedTools = ${
    scope.allowedTools === 'all' ? 'ALL' : `array of ${scope.allowedTools.length}`
  }`);
